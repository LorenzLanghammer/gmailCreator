...
async def main():
    ...
    with open("timezone_spoof.js", "r", encoding="utf-8") as f:
    ...
    await initial_tab.get("https://accounts.google.com/v3/signin/identifier?ifkv=AdBytiOJexdp1watCUyk4u_sQLe46DweG4nyViAnwUmHlUsKjm4Lcn1mvL7U2Pth-bfs1OsmBaHkEQ&flowName=GlifWebSignIn&flowEntry=ServiceLogin&dsh=S1191745681%3A1750467918808497")
    ...
    routines = [ 
        SelectAccountType_routine(initial_tab),
        EnterName_routine(initial_tab, name[0], name[1]), 
        EnterDateAndGender_routine(initial_tab, name[2]), 
        SelectAddressType_routine(initial_tab, country=country), 
        SelectAddress_routine(initial_tab, name[0], name[1]),
        EnterPassword_routine(initial_tab),
        EnterPhoneNumber_routine(initial_tab, country, browser),
        EnterCodeRoutine(initial_tab),
        DeclineRecoveryMailRoutine(initial_tab),
        DeclineRecoveryPhoneRoutine(initial_tab),
        DeclinePhoneUsageRoutine(initial_tab),
        ConfirmAccountCreationRoutine(initial_tab), 
        Personalization(initial_tab),
        ExpressSettingsRoutine(initial_tab),
        ConfirmSettingsRoutine(initial_tab),
        AgreeTosRoutine(initial_tab),
        SaveProfileRoutine(initial_tab, username)
        ]
